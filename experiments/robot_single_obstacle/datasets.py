...
def generate_symmetrical_points_for_fixed_circle(
    ...
    """
    Generates a single fixed circle and two symmetrical starting points for the robot_single_obstacle.
...
def generate_fixed_center_circle_and_point_v2(
    ...
    """
    Generates a valid circle and a starting point for the robot_single_obstacle with more control.
...
class RobotsDataset(CustomDataset):
    def __init__(self, random_seed, horizon, x0=torch.tensor([2., .5, 0, 0]), std_ini=0.2, n_agents=1):
        ...
        exp_name = 'robot_single_obstacle'
    ...
    def _generate_data(self, num_samples):
        ...
        # Concatenate robot_single_obstacle state and obstacle data
...
class RobotsDatasetMultiCircle_v2(CustomDataset):
    ...
    def _generate_data(self, num_samples):
        ...
        # 2. Create the final robot_single_obstacle state tensor
        data = torch.zeros(num_samples, self.horizon, state_dim)
        ...
        # 4. Concatenate robot_single_obstacle states and circle info
        Final_data = torch.cat((data, finalC), dim=2)
