...
class RobotsDatasetMulti(CustomDataset):
    def __init__(self, random_seed, horizon, N, vertices, center, radius, n_agents=1):
        # experiment and file names
        exp_name = 'robot'
        file_name = 'data_T' + str(horizon) + '_agents' + str(n_agents) + '_RS' + str(
            random_seed) + '.pkl'
        super().__init__(random_seed=random_seed, horizon=horizon, exp_name=exp_name, file_name=file_name)
    ...
    # ---- data generation ----
    def _generate_data(self, num_samples, max_attempts=10_000):
        """
        Generates N random points inside a square (defined by 4 vertices)
        but excluding points inside a given circle.
        ...
        """
        # Convert square to axis-aligned bounding box
        x_min, _ = torch.min(self.vertices[:, 0], dim=0)
        x_max, _ = torch.max(self.vertices[:, 0], dim=0)
        y_min, _ = torch.min(self.vertices[:, 1], dim=0)
        y_max, _ = torch.max(self.vertices[:, 1], dim=0)

        accepted = []
        attempts = 0

        while len(accepted) < self.N and attempts < max_attempts:
            # Generate in bulk
            batch_size = (self.N - len(accepted)) * 2
            samples = torch.empty(batch_size, 2).uniform_(0, 1)
            samples[:, 0] = samples[:, 0] * (x_max - x_min) + x_min
            samples[:, 1] = samples[:, 1] * (y_max - y_min) + y_min

            # Distance to circle center
            dist_sq = torch.sum((samples - self.center) ** 2, dim=1)
        ...
        return torch.tensor(accepted[:self.N])
